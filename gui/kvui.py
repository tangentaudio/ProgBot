...
class BoardDetailPopup:
    ...
    - Status of each phase (Vision, Contact, Program, Setup, Test)
    ...
    def _get_board_status(self, cell_id):
        ...
        if not self.app.bot or not hasattr(self.app.bot, 'board_statuses'):
            return None
        # Convert cell_id to (col, row) tuple used by board_statuses
    ...
    def _get_phase_color(self, status_name):
        ...
        if status_name in ("PASSED", "COMPLETED", "IDENTIFIED"):
            return [0.3, 0.8, 0.3, 1]  # Green
        elif status_name == "FAILED":
            return [0.9, 0.3, 0.3, 1]  # Red
        elif status_name in ("IN_PROGRESS", "RUNNING", "PROBING", "CAPTURING"):
    ...
    def show(self, cell, cell_id):
        ...
        self._phase_widgets = {}
        self._selected_phase = None
        self._phase_cards = {}  # Store card widgets for selection highlight
        
        # Build popup content
        content = BoxLayout(orientation='vertical', spacing=10, padding=10)
        
        # Header section - horizontal layout with board info, captures, and status
        ...
        # Failure reason box (always create, hide if no error)
        self._failure_box = BoxLayout(size_hint_y=None, height=50, padding=[10, 5])
        with self._failure_box.canvas.before:
            Color(0.6, 0.2, 0.2, 1)
            self._failure_box._rect = RoundedRectangle(pos=self._failure_box.pos, size=self._failure_box.size, radius=[5])
        ...
        # Left column - Phase status list (narrower)
        ...
        # Phase definitions: (label, dot_attr, enabled_attr, status_attr, timing_key)
        phases = [
        ...
        for phase_name, dot_attr, enabled_attr, status_attr, timing_key in phases:
            enabled = getattr(cell, enabled_attr, True)
            
            # Create clickable phase card (or non-clickable for disabled)
            if enabled:
                phase_card = ClickablePhaseCard(orientation='horizontal', size_hint_y=None, height=32, spacing=4, padding=[6, 2])
            else:
                phase_card = BoxLayout(orientation='horizontal', size_hint_y=None, height=32, spacing=4, padding=[6, 2])
            ...
            with phase_card.canvas.before:
                ...
                phase_card._rect = RoundedRectangle(pos=phase_card.pos, size=phase_card.size, radius=[5])
            ...
            # Timing label (right-aligned) - show "disabled" for disabled phases
        ...
        with detail_panel.canvas.before:
            ...
            detail_panel._rect = RoundedRectangle(pos=detail_panel.pos, size=detail_panel.size, radius=[8])
    ...
    def _update_content(self, dt=None):
        ...
        # Get timing data for this board
        board_times = {}
        if self.app.bot and hasattr(self.app.bot, 'stats') and self.app.bot.stats:
            board_times = self.app.bot.stats.board_times.get(position, {})
        ...
        if self._captures_label:
            ...
            if board_status and hasattr(board_status, 'board_info') and board_status.board_info:
                ...
                if test_data:
                    # Format captures as compact list
                    lines = ["[b]Captured:[/b]"]
        ...
        if self._failure_box and self._failure_label:
    ...
    def _update_detail_panel(self):
        ...
        status_attr, timing_key = phase_info[phase_name]
        ...
        # Build detail content based on phase
    ...
    def _on_rerun(self, instance):
        ...
        def do_rerun(dt):
            if self.app.bot:
                # Run cycle for just this one board
                import asyncio
...
class GridCell(ButtonBehavior, BoxLayout):
    ...
    def update_status(self, board_status):
        ...
        try:
            ...
            # Update is_active for pulsing animation
            is_processing = (
            ...
            if not board_status.enabled:
                ...
                self.cell_bg_color = [0, 0.5, 0, 1]  # Dark green when programmed
                ...
                self.cell_bg_color = [1, 1, 0, 1]  # Yellow while programming
    ...
    def _status_to_dot(self, status_name, enabled):
        """Convert a status name to a tuple of (dot_symbol, is_spinning)."""
        if not enabled:
            return (self.DOT_DISABLED, False)
        elif status_name in ("COMPLETED", "PASSED", "IDENTIFIED"):
            return (self.DOT_PASS, False)
        elif status_name == "FAILED":
            return (self.DOT_FAIL, False)
        elif status_name in ("IN_PROGRESS", "PROBING", "PROGRAMMING", "IDENTIFYING", "PROVISIONING", "TESTING"):
            return (self.SPINNER_FRAMES[self._spinner_index], True)
        elif status_name in ("IDLE", "PENDING"):
            return (self.DOT_PENDING, False)
        elif status_name == "SKIPPED":
            return (self.DOT_PENDING, False)  # Skipped - show empty box, not X
        else:
            return (self.DOT_PENDING, False)
    
    def _update_dots(self, board_status):
        """Update the status dots based on board status."""
        # Vision dot
        dot, spinning = self._status_to_dot(
...
class AsyncApp(SettingsHandlersMixin, PanelFileManagerMixin, ProvisionStepEditorMixin, RegexHelperMixin, App):
    ...
    def show_board_detail_popup(self, cell):
        """Show the board detail popup for a grid cell.
        
        Args:
            cell: The GridCell widget that was tapped
        """
        try:
            # Find cell ID from grid_cells dict
            cell_id = None
            for cid, c in self.grid_cells.items():
                if c is cell:
                    cell_id = cid
                    break
            
            if cell_id is None:
                log.warning("[BoardDetail] Could not find cell ID")
                return
            
            # Create popup controller if needed
            if not hasattr(self, 'board_detail_popup') or self.board_detail_popup is None:
                self.board_detail_popup = BoardDetailPopup(self)
            
            # Show the popup
            self.board_detail_popup.show(cell, cell_id)
            log.debug(f"[BoardDetail] Showing popup for cell {cell_id}")
            
        except Exception as e:
            log.error(f"Error showing board detail popup: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _reset_cell_status(self, cell, is_skipped=False):
        """Reset a single cell's status to initial state.
        
        Args:
            cell: The GridCell to reset
            is_skipped: Whether the cell is in the skip list
        """
        # Reset dot-based status fields
        cell.vision_dot = GridCell.DOT_DISABLED
        cell.contact_dot = GridCell.DOT_DISABLED
        cell.program_dot = GridCell.DOT_DISABLED
        cell.provision_dot = GridCell.DOT_DISABLED
        cell.test_dot = GridCell.DOT_DISABLED
        cell.serial_number = ""
        cell.failure_reason = ""
        cell.result_icon = ""
        cell.result_icon_color = [1, 1, 1, 1]
        
        # Stop any running animations
        cell._stop_spinner()
        cell._stop_pulse()
        
        # Set appearance based on skip state
        if is_skipped:
    ...
    def reset_grid(self, instance):
        ...
        def do_reset(dt):
            ...
            for cell_id, cell in self.grid_cells.items():
                ...
                self._reset_cell_status(cell, is_skipped)
    ...
    def stop(self, instance):
        ...
        try:
            ...
            # Stop all cell animations (spinners and pulsing)
            for cell in self.grid_cells.values():
                cell._stop_spinner()
                cell._stop_pulse()
    ...
    async def _do_start(self):
        ...
        # Reset active cells (not skipped) to initial state before starting
        for cell_id, cell in self.grid_cells.items():
            # Convert cell_id to [col, row]
            col = cell_id // self.grid_rows
            row_from_bottom = cell_id % self.grid_rows
            ...
            # Reset cell status (skipped cells stay black)
            self._reset_cell_status(cell, is_skipped)
        
        # Clear board statuses in bot before starting
        if self.bot:
            self.bot.board_statuses = {}
            self.bot.stats.reset()
        ...
        if (b := getattr(self, 'bot', None)):
            ...
            # Update grid cells with phase enabled flags
            for cell in self.grid_cells.values():
                cell.vision_enabled = new_config.vision_enabled
                cell.contact_enabled = new_config.programming_enabled  # Contact requires programming
                cell.program_enabled = new_config.programming_enabled
                cell.provision_enabled = new_config.provision_enabled
                cell.test_enabled = new_config.test_enabled
        else:
    ...
    async def _run_single_board(self, position):
        """Run the cycle for a single board position.
        
        Args:
            position: (col, row) tuple for the board to process
        """
        log.info(f"[SingleBoard] Running cycle for position {position}")
        
        if not self.bot:
            log.error("[SingleBoard] Bot not available")
            return
        
        # Check if a cycle is already running
        if hasattr(self, 'bot_task') and self.bot_task and not self.bot_task.done():
            log.warning("[SingleBoard] Cannot run - cycle already in progress")
            return
        
        # Disable UI during single-board run
        self._set_widget('start_button', disabled=True)
        self._set_widget('stop_button', disabled=False)
        self._set_config_widgets_enabled(False)
        self._set_grid_cells_enabled(False)
        ...
        try:
            # Reload config from current settings
            new_config = self._config_from_settings()
            self.bot.config = new_config
            
            # Update grid cell with phase enabled flags
            cell_id = position[0] * self.grid_rows + position[1]
            if cell_id in self.grid_cells:
                cell = self.grid_cells[cell_id]
                cell.vision_enabled = new_config.vision_enabled
                cell.contact_enabled = new_config.programming_enabled
                cell.program_enabled = new_config.programming_enabled
                cell.provision_enabled = new_config.provision_enabled
                cell.test_enabled = new_config.test_enabled
            
            # Reconnect stats signal
            try:
            ...
            # Run cycle for single board
            await self.bot.process_single_board(position)
            
        except Exception as e:
            log.error(f"[SingleBoard] Error: {e}")
            import traceback
            traceback.print_exc()
        finally:
            # Re-enable UI
            ...
            # Disconnect stats signal
            try:
                self.bot.stats_updated.disconnect(listener=self.on_stats_updated)
            except:
                pass
            
            log.info(f"[SingleBoard] Completed for position {position}")
